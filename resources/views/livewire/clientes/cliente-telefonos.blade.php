<div class="space-y-4">
    ...
    {{-- Flash --}}
    @if(session('success_telefonos'))
    <div class="text-sm text-green-700 bg-green-50 border border-green-200 rounded-lg px-4 py-2">
        {{ session('success_telefonos') }}
    </div>
    @endif

    {{-- Cabecera --}}
    <div class="flex items-center justify-between">
        <p class="text-sm text-gray-500">{{ $telefonos->count() }} teléfono(s) registrado(s)</p>
    ...
    <div class="bg-indigo-50 border border-indigo-200 rounded-xl p-5">
        <h4 class="text-sm font-semibold text-indigo-700 mb-4">
            {{ $editId ? 'Editar teléfono' : 'Nuevo teléfono' }}
        </h4>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">

            <div>
                <label class="block text-xs font-medium text-gray-700 mb-1">Número <span class="text-red-500">*</span></label>
                <input wire:model="numero" type="text" placeholder="Ej: 987654321"
            ...
            <div class="flex items-end">
                <div class="flex items-center gap-2">
                    <input wire:model="es_principal" type="checkbox" id="es_principal_t"
                           class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                    <label for="es_principal_t" class="text-sm text-gray-700">Principal</label>
                </div>
            </div>
    ...
    <div class="bg-white border border-gray-100 rounded-xl p-4 flex items-center justify-between gap-4 hover:border-gray-200 transition-colors">
        <div class="flex items-center gap-3">
            <div class="w-9 h-9 rounded-full flex items-center justify-center flex-shrink-0
                {{ $tel->tipo->value === 'whatsapp' ? 'bg-green-100 text-green-700' : ($tel->tipo->value === 'fijo' ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-600') }}">
                @if($tel->tipo->value === 'whatsapp')
            ...
            <div>
                <div class="flex items-center gap-2">
                    <span class="font-medium text-gray-800 text-sm">{{ $tel->numero }}</span>
                    @if($tel->es_principal)
                ...
                @if($tel->nombre)
                    <p class="text-xs text-gray-600 font-medium">{{ $tel->nombre }}</p>
                @endif
                <p class="text-xs text-gray-400">{{ $tel->tipo->label() }}</p>
            </div>
        </div>
        <div class="flex items-center gap-2 flex-shrink-0">
            @unless($tel->es_principal)
            <button wire:click="togglePrincipal({{ $tel->id }})"
                    class="text-xs text-gray-400 hover:text-indigo-600 underline" title="Marcar como principal">
                Principal
            </button>
            @endunless
            <button wire:click="editar({{ $tel->id }})"
            ...
            <button wire:click="eliminar({{ $tel->id }})"
                    wire:confirm="¿Eliminar este teléfono?"
